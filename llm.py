...
def ai_generate_chat(question, answers, role_answers):
    ...
    - 2. 圆桌会成员均为政治、国际形势、军事冲突、历史相关领域的专家，包括老白、牛总、小马哥、敏姐、单老板
    - 3. 五位专家首先一定要对自己输入的原始回答进行改写，使回答更加条理清楚、逻辑性强。同时改写后的答案要切合圆桌会议的讨论主题和氛围，绝对不可以是输入的原始答案的简单复述，也绝对不可以编造原始答案以外的内容
    - 4. 将优化后的回答作为自己的正式发言回答输出，每个专家只进行一次回答
    - 5. 在圆桌会议的讨论中，主持人针对不同专家答案要进行适当的串场，以引导继续讨论。引导发言时要明确点出专家的名字，这部分要由主持人即兴发挥
